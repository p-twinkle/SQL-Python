...
, TX AS
(
    SELECT user_id, visit_date, COUNT(amount) transactions_count
    FROM ADS 
    GROUP BY 1,2
)
, GS AS 
(
SELECT GENERATE_SERIES(0, MAX(transactions_count)) transactions_count
FROM TX
)

SELECT 
    GS.transactions_count, COALESCE(visits_count, 0) visits_count
FROM GS 
LEFT JOIN 
    (
